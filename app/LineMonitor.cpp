...
void LineMonitor::maimai() {
	...
	*/
	...
	if(this->balancingWalker->getStandControlMode() == true && lightStable == false && -0.03 <= lightDiff && lightDiff <= 0.03 && this->getAdjustedBrightness() < 0.85) { // 安定してない状態で安定&&脱線してないときグレー検知開始
		lightStableCount++;
